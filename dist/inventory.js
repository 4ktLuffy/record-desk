...
function renderInventoryRecord(){
  ...
  $('inventory-save-review').onclick=()=>run('Saving review history…',async()=>{inventoryReport=await api('/api/investigation-review',{run_id:r.id,record,decision:$('inventory-decision').value,note:$('inventory-note').value});renderInventory();$('inventory-record').value=String(record);renderInventoryRecord();notice('Review appended. Previous decisions remain in history.');});
}
$('inventory-refresh').onclick=()=>run('Refreshing inventory workspace…',async()=>{await inventoryLists();notice('Datasets and runs refreshed.');});
$('inventory-source').onchange=()=>run('Loading source columns…',async()=>{await inventoryColumns();notice('Map columns and confirm the stock contract.');});
$('inventory-demo').onclick=()=>run('Saving synthetic inventory CSV…',async()=>{const d=await api('/api/investigation-demo',{});await inventoryLists();await refreshDatasets();$('inventory-source').value=d.dataset_id;await inventoryColumns();for(const role of inventoryRoles)$('inventory-map-'+role).value=String(d.mapping[role]);$('inventory-train').value=d.train_end;$('inventory-calibration').value=d.calibration_end;$('inventory-end').value=d.test_end;$('inventory-contract').checked=false;notice('Synthetic source ready, including two invalid records. Check the mapping and confirm the daily stock contract.');});
$('inventory-run').onclick=()=>run('Validating source and saving investigation…',async()=>{if(!$('inventory-source').value)throw Error('Choose a saved CSV.');if(!$('inventory-contract').checked)throw Error('Confirm the stock contract first.');const mapping={};for(const role of inventoryRoles){const el=$('inventory-map-'+role);if(!el||el.value==='')throw Error('Map all seven fields.');mapping[role]=Number(el.value);}inventoryReport=await api('/api/investigation-run',{dataset_id:$('inventory-source').value,mapping,train_end:$('inventory-train').value,calibration_end:$('inventory-calibration').value,test_end:$('inventory-end').value,contract:'daily-whole-units-v1'});await inventoryLists();$('inventory-history').value=inventoryReport.id;renderInventory();notice('Investigation saved with source evidence and quarantine reasons.');});
$('inventory-history').onchange=()=>run('Opening saved investigation…',async()=>{if(!$('inventory-history').value)return;inventoryReport=await api('/api/investigation/'+$('inventory-history').value);renderInventory();notice('Saved investigation reopened.');});
