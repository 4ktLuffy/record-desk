...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if route=='/api/reconciliations':return self.respond(200,reconciliation.listing())
            if route.startswith('/api/reconciliation-bundle/'):return self.respond(200,reconciliation.bundle(route.rsplit('/',1)[1]),download='reconciliation-evidence.json')
            if route.startswith('/api/reconciliation/'):return self.respond(200,reconciliation.get(route.rsplit('/',1)[1]))
            ...
            static={'/':'index.html','/app.js':'app.js','/inventory.js':'inventory.js','/orders.js':'orders.js','/pipelines.js':'pipelines.js','/incidents.js':'incidents.js','/reconciliation.js':'reconciliation.js','/timeline.js':'timeline.js','/style.css':'style.css','/favicon.svg':'favicon.svg'}
            if route in static:
    ...
    def do_POST(self):
        ...
        try:
            ...
            if route=='/api/reconciliation-compare':result=reconciliation.compare(**data)
            elif route=='/api/reconciliation-repair':result=reconciliation.repair(**data)
            elif route=='/api/incident-simulate':result=incidents.simulate(**data)
